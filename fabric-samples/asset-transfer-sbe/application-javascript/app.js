...
function checkAsset(org, assetKey, resultBuffer, value, ownerOrg) {
    ...
    if (asset && value) {
        ...
        console.log(
            `*** Success from ${org} - asset ${assetKey} does not exist`,
        );
    } else {
...
async function readAssetByBothOrgs(
    assetKey,
    value,
    ownerOrg,
    contractOrg1,
    ...
    let resultBuffer;
    resultBuffer = await contractOrg1.evaluateTransaction(
        'ReadAsset',
        assetKey,
    );
    checkAsset('Org1', assetKey, resultBuffer, value, ownerOrg);
    resultBuffer = await contractOrg2.evaluateTransaction(
        'ReadAsset',
        assetKey,
    );
    ...
    try {
        ...
        try {
            ...
            try {
                ...
                console.log(
                    '*** Result: committed - because Org1 and Org2 both endorsed, while only the Org1 endorsement was required and checked',
                );
            } catch (updateError) {
                console.log(`*** Failed: update - ${updateError}`);
                process.exit(1);
            ...
            try {
                ...
                console.log(
                    '*** Result: committed - because Org1 was on the discovery list, Org2 did not endorse',
                );
            } catch (updateError) {
                console.log(`*** Failed: update - ${updateError}`);
                process.exit(1);
            ...
            try {
                ...
                console.log(
                    '*** Failed: committed - this should have failed to endorse and commit',
                );
            } catch (updateError) {
                console.log(
            ...
            try {
                ...
                console.log(
                    `*** Successfully caught the error: ${readDeleteError}`,
                );
            }
        } catch (runError) {
