...
describe("Chaincode", () => {
    ...
    describe("#BalanceOf", () => {
        it("should work", async () => {
            ...
            mockStub.getStateByPartialCompositeKey.resolves(
                new MockIterator(mockResponse),
            );
    ...
    describe("#TransferFrom", () => {
        ...
        beforeEach("Set up test parameters", () => {
            ...
            mockStub.createCompositeKey
                ...
                .returns("nft_101");
            mockStub.createCompositeKey
                .withArgs("balance", ["Alice", "101"])
                .returns("balance_Alice_101");
            mockStub.createCompositeKey
                .withArgs("balance", ["Bob", "101"])
                .returns("balance_Bob_101");
        });
        ...
        it("should work when a sender is the current owner", async () => {
            ...
            sinon.assert.calledWith(
                mockStub.putState,
                "nft_101",
                Buffer.from(JSON.stringify(updatedNft)),
            );
        ...
        it("should throw an error when a sender is invalid", async () => {
            ...
            await expect(
                token.TransferFrom(ctx, "Alice", "Bob", "101"),
            ).to.be.rejectedWith(
                Error,
        ...
        it("should throw an error when a current owner does not match", async () => {
            ...
            await expect(
                token.TransferFrom(ctx, "Charlie", "Bob", "101"),
    ...
    describe("#SetApprovalForAll", () => {
        it("should work", async () => {
            mockClientIdentity.getID.returns("Alice");
            mockStub.createCompositeKey
                .withArgs("approval", ["Alice", "Bob"])
                .returns("approval_Alice_Bob");
    ...
    describe("#IsApprovedForAll", () => {
        it("should work", async () => {
            ...
            mockStub.getState
                .withArgs("approval_Alice_Bob")
                .resolves(Buffer.from(JSON.stringify(approval)));
            ...
            const mockResponse = [
                {
                    key: "nft_101",
                    value: Buffer.from(
                        JSON.stringify({ tokenId: 101, owner: "Alice" }),
                    ),
                },
                {
                    key: "nft_102",
                    value: Buffer.from(
                        JSON.stringify({ tokenId: 102, owner: "Bob" }),
                    ),
                },
            ];
            mockStub.getStateByPartialCompositeKey.resolves(
    ...
    describe("#MintWithTokenURI", () => {
        it("should work with a new token", async () => {
            mockClientIdentity.getMSPID.returns("Users");
            mockClientIdentity.getID.returns("Alice");
            ...
            const response = await token.MintWithTokenURI(
                ctx,
                "101",
                "DummyURI",
            );
        ...
        it("should throw an error when a tokenId alreay exists", async () => {
            ...
            await expect(
                token.MintWithTokenURI(ctx, "mytoken1", "DummyURI"),
            ).to.be.rejectedWith(
                Error,
                "The token mytoken1 is already minted.",
            );
        });
        ...
        it("should throw an error when a tokenId is not an integer", async () => {
            ...
            await expect(
                ...
                Error,
                "The tokenId mytoken1 is invalid. tokenId must be an integer",
            );
        });
    });
    ...
    describe("#Burn", () => {
        it("should work", async () => {
            ...
            sinon.assert.calledWith(
                mockStub.deleteState.getCall(1),
                "balance_Bob_101",
            );
            expect(response).to.equals(true);
        });
